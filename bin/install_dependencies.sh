...
if [[ -z "${CONAN_PROFILE}" ]]
then
    CONAN_PROFILE_FLAGS="-s compiler=$(basename "${CC}") -s compiler.libcxx=libstdc++11"
else
    CONAN_PROFILE_FLAGS="--profile ${CONAN_PROFILE}"
fi
...
conan install "$conan_file" ${CONAN_PROFILE_FLAGS}
