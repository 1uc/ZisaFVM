...
std::shared_ptr<AllVariables> RayleighTaylor::compute_initial_conditions(
  ...
  //
  //  auto theta_inner = eos->enthalpy_entropy(rhoP);
  //  auto theta_outer = eos->enthalpy_entropy(RhoP{rhoP.rho() + drho,
  //  rhoP.p()});
  //
  //  auto ic_ = PolytropeWithJumpIC(eos, gravity, theta_inner, theta_outer,
  //  x_ref);
  //  auto ic = [](const auto &) {
  //    auto v = 0.0;
  //
  ...
  //  };
  //
  //  auto tracer_ic = [](const auto &x) { return 1.0; };
  //
  //  auto qr = choose_volume_rule();
  //  auto &u0 = all_variables->cvars;
  //  auto &q0 = all_variables->avars;
